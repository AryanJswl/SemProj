...
function InvestorItem(props) {
  ...
  async function Connect() {
    // let response;
    await fetch("http://65.1.106.123:8080/investor/addproposal", {
      method: "PUT",
      body: JSON.stringify({
  ...
  async function chathandler() {
    ...
    let response, data;
    try {
      response = await fetch("http://65.1.106.123:8080/conversation/getChat", {
        method: "POST",
        body: JSON.stringify({
          id1: id1,
          id2: id2,
      ...
      if (data.length === 0) {
        let resp, Data;
        try {
          resp = await fetch("http://65.1.106.123:8080/conversation/", {
            method: "POST",
            body: JSON.stringify({
              senderId: id1,
              receiverId: id2,
    ...
    <>
      <li className={classes.item}>
        <div className={classes.img}>
          <img
            src={"http://65.1.106.123:8080/" + `${props.image}`}
            alt={props.name}
