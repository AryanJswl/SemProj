...
function Messages(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      socket.current = io.connect("ws://65.1.106.123:8900");
  ...
  useEffect(() => {
    const unSub = async () => {
      let response, data;
      try {
        response = await fetch("http://65.1.106.123:8080/messages/getmessages", {
          method: "POST",
          body: JSON.stringify({
...
// const encrypted = publicKey.encrypt(text, 'base64');
    ...
    let response;
    try {
      response = await fetch("http://65.1.106.123:8080/messages/newmessage", {
        method: "POST",
        body: JSON.stringify({
          conversationId: ctx.conversationId.conversationId,
