...
function Messages(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      socket.current = io.connect("ws://3.110.208.9:8900");
  ...
  useEffect(() => {
    const unSub = async () => {
      let response, data;
      try {
        response = await fetch("http://3.110.208.9:8080/messages/getmessages", {
          method: "POST",
          body: JSON.stringify({
...
// const encrypted = publicKey.encrypt(text, 'base64');
    ...
    let response;
    try {
      response = await fetch("http://3.110.208.9:8080/messages/newmessage", {
        method: "POST",
        body: JSON.stringify({
          conversationId: ctx.conversationId.conversationId,
